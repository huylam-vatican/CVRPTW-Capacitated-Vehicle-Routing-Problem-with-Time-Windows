   ...
   Constraint_05:
    forall (k in truckth, i in I, j in J: i != j)
        if (j != 0) {
            ...
            s[k][i] + st[i] + t[i][0] <= b[0]; // Modified to ensure returning to depot does not violate the depot's closing time
}
    ...
    for (var k in truckth) {
        for (var i in I) {
            for (var j in J) {
                if (x[k][i][j] == 1) {
                    if (j != 0) {
                        var tripStartTime = s[k][j] - t[i][j]; // Normal nodes
                         var serviceStartTime = s[k][j];
                    var serviceEndTime = s[k][j] + st[j];
                    } else {
                        tripStartTime = se[k][0] - t[i][0]; // Returning to depot
                        serviceStartTime = se[k][0]; // When the truck actually arrives back at the depot
